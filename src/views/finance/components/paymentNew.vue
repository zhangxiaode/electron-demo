<template>
  <n-modal
    ...
    preset="dialog"
    :title="props.id ? '编辑付款' : '新增付款'"
    :show-icon="false"
    @update:show="updateShow"
    style="width: 1000px;" 
  >
    <template #header>
      <slot name="header">{{ props.id ? '编辑付款' : '新增付款' }}</slot>
    </template>
    <slot>
      <n-form
        class="form"
        ...
        <div class="flex jc-between">
          <div class="flex-1">
            <n-form-item label="应付总额（元）：" path="yfMoeny">
              <n-input-number type="number" class="n_input" v-model:value="form.yfMoeny" :show-button="false" placeholder="请输入应付总额">
                <template #prefix>¥</template>
              </n-input-number>
            </n-form-item>
            <n-form-item label="未付金额（元）：" path="unPayMoeny">
              <n-input-number type="number" class="n_input" v-model:value="form.unPayMoeny" :show-button="false" placeholder="请输入未付金额">
                <template #prefix>¥</template>
              </n-input-number>
            </n-form-item>
          ...
          <div class="flex-1">
            <n-form-item label="付款金额（元）：" path="payMoeny">
              <n-input-number type="number" class="n_input" v-model:value="form.payMoeny" :show-button="false" placeholder="请输入付款金额">
                <template #prefix>¥</template>
              </n-input-number>
            </n-form-item>
...
import { getDictList, addFkpayment, editFkpayment, getFkpaymentDetail } from "@/apis";
