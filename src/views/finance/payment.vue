<template>
  <div class="main-pad h-100 flex flex-column jc-between">
    ...
    >
      ...
      <n-form-item v-permission="'financial:zf:add'">
        <n-button type="primary" @click="onAdd">新增</n-button>
      </n-form-item>
      <n-form-item v-permission="'financial:zf:export'">
    ...
    <PaymentNew v-if="showNewPayment" :id="id" :visable="showNewPayment" @close="handleClosePayment" />
    <PaymentDetail v-if="showDetailPayment" :id="id" :visable="showDetailPayment" @close="handleClosePaymentDetail" />
  </div>
</template>
...
import { getFkpaymentList, deleteFkpayment, exportFkpayment, changeFkpaymentStatus } from "@/apis/index";

let showNewPayment = ref(false);
let showDetailPayment = ref(false);
let id = ref(null);
    ...
    render(row: any, index: number) {
      return [
        ...
        withDirectives(h(NButton, {
          ...
          size: "small",
          onClick: () => onEdit(row)
        }, {default: () => '编辑'}), [[resolveDirective('permission') as any, 'financial:zf:edit']]),
        withDirectives(h(NButton, {
          type: 'primary',
          link: true,
          quaternary: true,
          size: "small",
          onClick: () => onDel(row)
        }, {default: () => '删除'}), [[resolveDirective('permission') as any, 'financial:zf:remove']])
      ]
    }
  }
]);
...
const onDetail = (row: any) => {
  id.value = row.id
  showDetailPayment.value = true
};
const handleClosePayment = async () => {
  showNewPayment.value = false
  getList();
}
const handleClosePaymentDetail = async () => {
  showDetailPayment.value = false
}
const onReturn = () => {
  router.back()
};
const onExport = () => {
  downloadFile('/api/pc/system/fkpayment/export', 'GET')
};
const onDel = (row: any) => {
  dialog.warning({
    title: '提示',
    content: () => h('div', {}, { default: () => '确认删除当前付款记录？' }),
    positiveText: '确定',
    negativeText: '取消',
...
const getList = async () => {
  try {
    loading.value = true
    const res = await getFkpaymentList({
